"""
Taken from: https://github.com/learn2phoenix/CSD/search/embeddings.py
"""
...
class Embeddings(object):
    ...
    def __init__(self, data_dir, chunk_dir, file_ext='.pt', files=None, chunked=False, chunk_size=5000):
        ...
        if files is not None:
            self.embedding_files = list(
                map(lambda x: osp.join(data_dir, x), files))
        else:
        ...
        if not self.chunked:
            threading.Thread(target=self.__result_consumer,
                             daemon=True).start()
            self.__read_embeddings()
            self.embeddings, self.filenames = self.__remove_missing(
                self.embeddings, self.filenames)
        else:
        ...
        while True:
            ...
            if not isinstance(data, str):
                ...
                if len(embedding_chunk) < self.chunk_size:
                    ...
                    chunk_cnt += 1
                    embedding_chunk, filename_chunk = self.__remove_missing(
                        embedding_chunk, filename_chunk)
                ...
                embedding_chunk, filename_chunk = self.__remove_missing(
                    embedding_chunk, filename_chunk)
                save_chunk(embedding_chunk, filename_chunk,
                           chunk_cnt, self.chunk_dir)
                # module_logger.info(
    ...
    def __read_embeddings_chunked(self):
        ...
        print(f'{self.chunk_dir}/*{self.file_ext}')

        files = glob.glob(f'{self.chunk_dir}/*{self.file_ext}')
        cnt = 0
        with concfut.ProcessPoolExecutor(max_workers=32) as executor:
            futures = [executor.submit(load_chunk, filename)
                       for filename in files]
            for future in concfut.as_completed(futures):
                result = future.result()
                self.embeddings.extend(
                    list(map(lambda x: x.squeeze(), result['embeddings'])))
                self.filenames.extend(result['filenames'])
                cnt += 1
            print(f'Finished reading {len(self.filenames)} embeddings')
    ...
    def __remove_missing(self, embeddings, filenames):
        ...
        embeddings = [ele for idx, ele in enumerate(
            embeddings) if idx not in missing_ids]
        filenames = [ele for idx, ele in enumerate(
            filenames) if idx not in missing_ids]
...
def save_chunk(embeddings, filenames, count, chunk_dir, chunk_size=50000):
    ...
    if len(embeddings) < chunk_size:
        ...
        pickle.dump(data, open(
            osp.join(chunk_dir, f'embeddings_{count}.pkl'), 'wb'))
    else:
