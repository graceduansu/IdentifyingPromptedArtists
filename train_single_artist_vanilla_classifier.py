...
from models.vanilla_classifier import ClipClassifier


def parse_args(input_args=None):
    """
    Parses command-line arguments used for configuring a session.
   ...
   """
    ...
    parser.add_argument(
        "--dataset_info_dir", default="metadata/dataset_sdxl_simple_prompts", type=str)
    ...
    parser.add_argument("--tracker_project_name", type=str,
                        default="vanilla_classifier", help="The name of the wandb project to log to.")
    parser.add_argument("--run_name", type=str,
                        default="vanilla_classifier", help="The name of the wandb run.")
...
def validation(model, val_loader, criterion, device="cuda"):
    """Validation function for vanilla classifier"""
    val_log = {}
...
def main(args):
    ...
    print(f"Using test_dataset_info_dir: {args.test_dataset_info_dir}")

    val_data = ArtistDataset(
        split=args.test_dataset_split,
        image_prep="clip_base_noaug",
    ...
    model = ClipClassifier(num_classes=num_classes, model_arch=args.model_arch)
    ...
    # cast model
    model = model.to(dtype=torch.float32, device=accelerator.device)
    ...
    if args.resume_from_ckpt is not None:
        ...
        init_kwargs = {
            "wandb": {"dir": "./wandb", "resume": "must", "id": args.wandb_resume_id}
        }
    else:
        init_kwargs = {
            "wandb": {
